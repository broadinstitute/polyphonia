...
if(!scalar @ARGV) # no command line arguments supplied
{
	...
	print STDOUT "\t-2 | --print-all-iSNVs BOOL\tInclude in plate visualization file all threshold-passing samples, including samples without plate neighbors [".int_to_bool_string($DEFAULT_PRINT_ALL_ISNVS)."]\n";
